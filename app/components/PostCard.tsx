...
// Only show images for substantial articles (deep dives, 10+ min)
function shouldShowImage(readingTime: number): boolean {
...
export default function PostCard({ post }: { post: Post }) {
  ...
  const showImage = shouldShowImage(post.readingTime);
  const imageUrl = post.metadata.image || `/images/posts/${post.slug}.jpg`;

  return (
    <Link href={`/posts/${post.slug}`}>
      <article className="group bg-zinc-900 border border-zinc-800 hover:border-zinc-600 transition-colors relative overflow-hidden flex flex-col h-full">
        {/* Hero Image - only for deep dive articles */}
        {showImage && (
        ...
        <div className={showImage ? 'p-6 flex flex-col flex-grow' : 'p-8 flex flex-col flex-grow'}>
          {hasRead && (
          ...
          <h2 className="font-serif text-2xl font-light text-white mb-3 group-hover:text-gray-300 transition-colors">
            {post.metadata.title}
          </h2>
          <p className="text-gray-400 leading-relaxed mb-4 flex-grow">
            {post.metadata.excerpt}
          </p>
          <div className="text-gray-500 text-sm flex items-center gap-3 mt-auto">
            <span>
